...
struct OBJParseContext {
    ...
    OBJParseContext(RawMeshData& m)
        : mesh(m) {}
};

/*
*  Forward Declarations - FIXED: All should use RawMeshData*
*/
...
inline void generateUVs(RawMeshData* mesh);  // FIXED: RawMeshData* not Mesh*
inline void computepackedTNBFrame(RawMeshData* mesh);  // FIXED: RawMeshData* not Mesh*

/*
*  Main OBJ Loading
*/
static RawMeshData* loadOBJ(const std::string& fileContent) {
    if (fileContent.empty()) {
        std::cerr << "[Error] OBJLoader::loadOBJ: File content is empty\n";
        return nullptr;
    }

    RawMeshData* mesh = new RawMeshData();
    ...
    // FIXED: Now correctly passes RawMeshData*
...
/*
*  Mesh data generators - FIXED: All use RawMeshData*
*/
inline void generateUVs(RawMeshData* mesh) {  // FIXED: Parameter type
    mesh->uvs.clear();
...
inline void packTBNframe(RawMeshData* mesh, const std::vector<glm::vec3>& normals,
             const std::vector<glm::vec4>& tangents) {  // FIXED: Parameter type
...
void computepackedTNBFrame(RawMeshData* mesh) {  // FIXED: Parameter type
    ...
    // FIXED: Call the helper function to actually pack the data
